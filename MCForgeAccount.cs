...
namespace MCForge
{
    public static class MCForgeAccount
    {
        ...
        public static bool Login(string user, string pass)
        {
            ...
            {
                LoggedIn = false;
                Server.s.Log("Logging into MCWall.tk...");
                byte[] data = Encoding.ASCII.GetBytes(string.Format("action=do_login&username={0}&password={1}", user, pass));

                HttpWebRequest request = (HttpWebRequest)WebRequest.Create("http://mcwall.bugs3.com/concrete5.6.2/index.php/login/");
                ...
                using (Stream stream = response.GetResponseStream())
                {
                    using (StreamReader reader = new StreamReader(stream))
                    {
                        ...
                        {
                            Server.s.Log("Successfully logged into mcwall.tk!");
