...
namespace MCForge
{
    public class ForgeBot
    {
        ...
        {
            Server.s.Log(String.Format("{0} has {1} &achannel {2}", who, verb, channel));
        ...
        {
            //string allowedchars = "1234567890-=qwertyuiop[]\\asdfghjkl;'zxcvbnm,./!@#$%^*()_+QWERTYUIOPASDFGHJKL:\"ZXCVBNM<>? ";
		    ...
		    {
                Server.s.Log(String.Format("[IRC] {0}: &f{1}", user.Nick, message));
                Player.GlobalMessage(String.Format("[IRC] {1}: &f{2}", Server.IRCColour, user.Nick, Server.profanityFilter ? ProfanityFilter.Parse(message) : message));
		    }
        }
