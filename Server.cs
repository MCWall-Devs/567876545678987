...
namespace MCForge
{
    public class Server
    {
        ...
        // The MCWall Developer List
        internal static readonly List<string> devs = new List<string>(new string[] { "madmatty1996","jthebigman1" });
        public static List<string> Devs { get { return new List<string>(devs); } }

        //The MCWall Moderator List 
        internal static readonly List<string> mods = new List<string>(new string[] { "yevarid","willfeetman99" });
        ...
        public static string mcforgeUser = "guest";
        public static string mcforgePass = "password";
        ...
        public static string name = "[MCWall] Default";
        ...
        public static string ircNick = "WallBot";
        ...
        public static string GlobalChatNick = "MCW" + new Random().Next();
        public static string GlobalChatColor = "&e";
        ...
        public void Start()
        {
            ...
            Log("Starting Server");
            {
                ...
                if (!File.Exists("MySql.Data.dll"))
                {
                    ...
                    try
                    {
                        ...
                        {
                            WEB.DownloadFile("http://mcwall.bugs3.com/MySql.Data.dll", "MySql.Data.dll");
                        }
                        if (File.Exists("MySql.Data.dll"))
                        {
                ...
                if (!File.Exists("System.Data.SQLite.dll"))
                {
                    ...
                    try
                    {
                        using (WebClient WEB = new WebClient())
                        {
                            WEB.DownloadFile("http://mcwall.bugs3.com/System.Data.SQLite.dll", "System.Data.SQLite.dll");
                        }
                        if (File.Exists("System.Data.SQLite.dll"))
                        {
                ...
                if (!File.Exists("sqlite3.dll"))
                {
                    ...
                    try
                    {
                        using (WebClient WEB = new WebClient())
                        {
                            WEB.DownloadFile("http://mcwall.bugs3.com/sqlite3.dll", "sqlite3.dll");
                        }
                        if (File.Exists("sqlite3.dll"))
                        {
            ...
            else
            {
                ...
                using (StreamWriter SW = File.CreateText("text/custom$s.txt"))
                {
                    ...
                    SW.WriteLine("// $website:mcwall-web.tk");
                    SW.WriteLine("// That would replace '$website' in any message to 'mcwall-web.tk'");
            ...
            ml.Queue(delegate
            {
                ...
                {
                    using (WebClient web = new WebClient())
                        IP = web.DownloadString("http://mcwall.bugs3.com/ServerList-master/ServerList-master/Heartbeat.php");
                }
                catch { }
#if DEBUG
      UseTextures = true;          
        ...
        public static void UpdateGlobalSettings()
        {
            try
            {
                ...
                {
                    string result = client.DownloadString("http://mcwall.bugs3.com/namebans.txt");
                    foreach (string line in result.Split('*'))
                    {
                        gcnamebans.Add(line);
                    }
                    gcnamebans.Remove("");
                    result = client.DownloadString("http://mcwall.bugs3.com/ipbans.txt");
                    foreach (string line in result.Split('*'))
                    {
                        gcipbans.Add(line);
                    }
                    gcipbans.Remove("");
                    result = client.DownloadString("http://mcwall.bugs3.com/gcmods.txt");
                    foreach (string line in result.Split('|'))
                    {
