        ...
        public override void Use(Player p, string message)
        {
            ...
            if (p != null)
            {
                foreach (string line in Server.gcnamebans)
                {
                    ...
                    {
                        Player.SendMessage(p, "You have been banned from the global chat by " + line.Split('|')[2] + " because of the following reason: " + line.Split('|')[1] + ". You can apply a ban appeal at mcwall.tk. Keep yourself to the rules.");
                        return;
                    }
                }
                foreach (string line in Server.gcipbans)
                {
                    ...
                    {
                        Player.SendMessage(p, "You have been ip banned from the global chat by " + line.Split('|')[2] + " because of the following reason: " + line.Split('|')[1] + ". You can apply a ban appeal at mcwall.tk. Keep yourself to the rules.");
                        return;
                    }
                }
            }            Server.GlobalChat.Say((p != null ? p.name + ": " : "Console: ") + message, p);
            Player.GlobalMessage(Server.GlobalChatColor + "<&e[Global] " + (p != null ? p.name + ": " : "&9Console: ") + "&f" + (Server.profanityFilter ? ProfanityFilter.Parse(message) : message), true);
