...
namespace MCForge.Commands
{
    class CmdObUpdate : Command
    {
        ...
        public override string shortcut { get { return new string[] { }; } }
        ...
        public override void Use(Player p, string message)
        {
            ...
            foreach (Player pl in Player.players)
            {
                ...
                if (Server.OmniBanned().Contains(pl.name.ToLower()) || Server.OmniBanned().Contains(pl.ip) || omniBanned)
                    pl.Kick("You have been Omni-banned. Visit mcwall.tk for appeal."); 
            }
            Player.SendMessage(p, "MCWall Omni-Ban List updated.");
        }

        public override void Help(Player p)
        {
            Player.SendMessage(p, "/obupdate - Updates the MCWall Omni-Ban list.");
