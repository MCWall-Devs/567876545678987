                ...
                switch (message)
                {
                    case "":
                        ...
                        else
                        {
                            ...
                            Player.SendMessage(p, "use &b/help grief" + Server.DefaultColor + " for a list of commands to stop griefers.");
                            Player.SendMessage(p, "Use &b/help voting" + Server.DefaultColor + " for a list of voting commands.");
                            Player.SendMessage(p, "Use %b/help fun" + Server.DefaultColor + " for a list of fun commands.");
                            ...
                            Player.SendMessage(p, "Use &b/help custom" + Server.DefaultColor + " for a list of Custom commands.");
                    ...
                    case "voting":
                        ...
                        foreach (Command comm in Command.all.commands)
                        {
                            if (p == null || p.group.commands.All().Contains(comm))
                            {
                                if (comm.type.Contains("voting")) message += ", " + getColor(comm.name) + comm.name;
                            }
                        }

                        if (message == "") { Player.SendMessage(p, "No commands of this type are available to you."); break; }
                        Player.SendMessage(p, "Voting commands you may use:");
                        Player.SendMessage(p, message.Remove(0, 2) + ".");
                        break;
                    case "fun":
                        message = "";
                        foreach (Command comm in Command.all.commands)
                        {
                            if (p == null || p.group.commands.All().Contains(comm))
                            {
                                if (comm.type.Contains("fun")) message += ", " + getColor(comm.name) + comm.name;
                            }
                        }

                        if (message == "") { Player.SendMessage(p, "No commands of this type are available to you."); break; }
                        Player.SendMessage(p, "Fun commands you may use:");
                        Player.SendMessage(p, message.Remove(0, 2) + ".");
                        break;
                    case "grief":
                        message = "";
                        foreach (Command comm in Command.all.commands)
                        {
                            if (p == null || p.group.commands.All().Contains(comm))
                            {
                                if (comm.type.Contains("grief")) message += ", " + getColor(comm.name) + comm.name;
                            }
                        }

                        if (message == "") { Player.SendMessage(p, "No commands of this type are available to you."); break; }
                        Player.SendMessage(p, "Grief commands you may use:");
                        Player.SendMessage(p, message.Remove(0, 2) + ".");
                        break;
                    ...
                    case "custom":
                        ...
                        foreach (Command comm in Command.all.commands)
                        {
                            if (p == null || p.group.commands.All().Contains(comm))
                            {
                                if (comm.type.Contains("custom")) message += ", " + getColor(comm.name) + comm.name;
                            }
                        }

                        if (message == "") { Player.SendMessage(p, "No commands of this type are available to you."); break; }
                        Player.SendMessage(p, "Custom commands you may use:");
                        Player.SendMessage(p, message.Remove(0, 2) + ".");
                        break;
                    default:
