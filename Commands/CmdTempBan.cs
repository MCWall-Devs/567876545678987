    ...
    public class CmdTempBan : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (Server.devs.Contains(who.name.ToLower()))
            {
                ...
                if (p != null)
                {
                    Player.GlobalMessage(p.color + p.name + Server.DefaultColor + " attempted to tempban a MCWall Developer!");
                }
                else
                {
                    Player.GlobalMessage(Server.DefaultColor + "The Console attempted to tempban a MCWall Developer!");
                }
                return;
            }
            int minutes;
