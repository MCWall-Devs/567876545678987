...
namespace MCForge_.Gui
{
    public static class Program
    {
        ...
        private static string DLLLocation = "http://mcwall.bugs3.com/MCForge_.dll";
        private static string ChangelogLocation = "http://www.mcforge.net/changelog.txt";
        private static string EXELocation = "http://mcwall.bugs3.com/MCWall.exe";
        //private static string RevisionList = "http://www.mcforge.net/revs.txt";
        //private static string HeartbeatAnnounce = "http://mcwall.bugs3.com/ServerList-master/ServerList-master/Heartbeat.php";
        ...
        public static void Main(string[] args)
        {
            ...
            if (Process.GetProcessesByName("MCWall").Length != 1)
            {
                foreach (Process pr in Process.GetProcessesByName("MCWall"))
                {
        ...
        remake:
            try
            {
                ...
                if (foundView[4].Split(' ')[2].ToLower() == "true")
                {
                    ...
                    Console.Title = Server.name + " - MCWall " + Server.Version;
                    usingConsole = true;
                    handleComm();
        ...
        public static void PerformUpdate()
        {
            try
            {
                ...
                try
                {
                    ...
                    if (File.Exists("MCWall_.update"))
                        File.Delete("MCWall_.update");
                    if (File.Exists("MCWall.update"))
                        File.Delete("MCWall.update");
                    if (File.Exists("Update.bat"))
                ...
                WebClient Client = new WebClient();
                Client.DownloadFile(DLLLocation, "MCWall_.update");
                Client.DownloadFile(EXELocation, "MCWall.update");
